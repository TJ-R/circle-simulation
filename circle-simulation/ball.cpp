...
Ball::Ball(int newId, float radius) : CircleShape(radius) {
	this->id = newId;
}

void Ball::setId(int newId) {
	this->id = newId;
}
...
void Ball::checkBorderCollision(sf::RenderWindow* window) {
	// TODO: Update to not use magic when reducing speeds and ensuring collsion with border does not exit
	// the wall
	if (this->AABB_Collider.getPosition().x < 0) 
	{
		this->setPosition({ 0+10, this->getPosition().y });
		this->setMovementVector({ -(this->getMovementVector().x * .5f), this->getMovementVector().y });
		sf::Vector2f circleCenter = this->getPosition() + this->getGeometricCenter();
		this->AABB_Collider.setPosition(circleCenter - this->AABB_Collider.getGeometricCenter());
	}
	else if (this->AABB_Collider.getPosition().x + this->AABB_Collider.getSize().x > window->getSize().x) 
	{
		this->setPosition({ static_cast<float>(window->getSize().x) - (this->getRadius() * 2) - 10, this->getPosition().y});
		this->setMovementVector({ -(this->getMovementVector().x * .5f), this->getMovementVector().y });
		sf::Vector2f circleCenter = this->getPosition() + this->getGeometricCenter();
		this->AABB_Collider.setPosition(circleCenter - this->AABB_Collider.getGeometricCenter());
	...
	if (this->AABB_Collider.getPosition().y < 0) 
	{
		...
		this->setMovementVector({ this->getMovementVector().x, -(this->getMovementVector().y * .5f) });
		sf::Vector2f circleCenter = this->getPosition() + this->getGeometricCenter();
		this->AABB_Collider.setPosition(circleCenter - this->AABB_Collider.getGeometricCenter());

	}
	else if (this->AABB_Collider.getPosition().y + this->AABB_Collider.getSize().y > window->getSize().y)
	{
		if (this->getMovementVector().y < 10.f) {
			this->setPosition({ this->getPosition().x, static_cast<float>(window->getSize().y) - (this->getRadius() * 2)});
		}
		else {
			//this->setPosition({ this->getPosition().x, static_cast<float>(window->getSize().y) - (this->getRadius() * 2) - 10 });
			this->setPosition({ this->getPosition().x, static_cast<float>(window->getSize().y) - (this->getRadius() * 2) - 3.f});
		}
		this->setMovementVector({ this->getMovementVector().x, -(this->getMovementVector().y * .5f) });
		sf::Vector2f circleCenter = this->getPosition() + this->getGeometricCenter();
		this->AABB_Collider.setPosition(circleCenter - this->AABB_Collider.getGeometricCenter());
...
bool Ball::checkParticleCollision(Ball* ball) {
		...
		// Get Eucldiean distance between the two centers (x2 - x1)^2 + (y2 - y1)^2 = radii^2
...
void Ball::resolveParticleCollision(Ball* ball, float dt) {
	...
	sf::Vector2f normalVec = sf::Vector2f(b2Center.x - b1Center.x, b2Center.y - b1Center.y);
...
void Ball::handleCollisions (sf::RenderWindow* window, Ball* ballArr, int ballArrSize, float dt) {
...
void Ball::update(float dt) 
{
	...
	// Apply 
	this->setMovementVector({ this->getMovementVector().x, this->getMovementVector().y + (15.f * dt) });
