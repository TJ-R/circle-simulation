...
Ball::Ball(float radius, int windowX, int windowY, float restitiution,
    ...
    this->setMovementVector(sf::Vector2f(velX, velY));
...
void Ball::createRandomBall(float posX, float posY) {
    sf::Color colors[7] = {sf::Color::White,  sf::Color::Red,
...
void Ball::setMovementVector(sf::Vector2f vector) {
    movementVector = vector;
}

sf::Vector2f Ball::getMovementVector() {
    return this->movementVector;
}

void Ball::setResitiutionCoefficient(float e) {
    restitution = e;
}

sf::RectangleShape &Ball::getAABBCollider() {
...
int Ball::checkBorderCollisions(sf::RenderWindow *window) {
    ...
    if (this->AABB_Collider.getPosition().x < 0) {
        ...
        this->setMovementVector(
            {-(this->getMovementVector().x * this->restitution),
             this->getMovementVector().y});
        sf::Vector2f circleCenter =
            this->getPosition() + this->getGeometricCenter();
        this->AABB_Collider.setPosition(
            circleCenter - this->AABB_Collider.getGeometricCenter());
    } else if (this->AABB_Collider.getPosition().x +
                   this->AABB_Collider.getSize().x >
               window->getSize().x) {
        this->setPosition({static_cast<float>(window->getSize().x) -
                               (this->getRadius() * 2) - 10,
                           this->getPosition().y});
        this->setMovementVector(
            {-(this->getMovementVector().x * this->restitution),
...
void Ball::resolveBorderCollisions(sf::RenderWindow *window, float dt,
    ...
    case Top:
    case Bottom: {
        float oldX = this->getMovementVector().x;
        float oldY = this->getMovementVector().y;
        sf::Vector2f *newMovVec = new sf::Vector2f(oldX, -1 * oldY);
        this->setMovementVector(*newMovVec);
        break;
    }
    case Right:
    case Left: {
        float oldX = this->getMovementVector().x;
        float oldY = this->getMovementVector().y;
        sf::Vector2f *newMovVec = new sf::Vector2f(-1 * oldX, oldY);
        this->setMovementVector(*newMovVec);
        break;
    }
    case TR:
    ...
    case TL:
        float oldX = this->getMovementVector().x;
        float oldY = this->getMovementVector().y;
        sf::Vector2f *newMovVec = new sf::Vector2f(-1 * oldX, -1 * oldY);
        this->setMovementVector(*newMovVec);
        break;
    }
}
...
void Ball::staticCollisions(sf::RenderWindow *window, float dt) {
    ...
    //
    //
    // Going to do some position updates in here even though it probably should
    // be done here
    ...
    float mSumTop =
        (p_top - this->getPosition().y) / this->getMovementVector().y;
    float mSumBot =
        (p_bot - this->getPosition().y) / this->getMovementVector().y;
    float mSumLeft =
        (p_left - this->getPosition().x) / this->getMovementVector().x;
    float mSumRight =
        (p_right - this->getPosition().x) / this->getMovementVector().x;

    // Need to determine if any direction if collision is going to happen and
    // then approach it based on which is first Might be good to do some kind of
    // array real quick and only push the ones that are postitive with new
    // object type that tracks the collision code and the time t then solve the
    // min the correct order
    //
    // For example if t=.7 for bottom and  do bottom first for 70% of the frame.
    // Update the position onto the bottom and the vector. Then handle the
    // second wall that was at t=.9 and update the movement vector. Then do the
    // last 10% of the collsion in the final movement direction. Would need a
    // helper function update the positon based on t and dt
}
...
void Ball::resolveParticleCollision(Ball *ball, float dt) {
    ...
    sf::Vector2f unitTangentVec =
        sf::Vector2f(-unitNormalVec.y, unitNormalVec.x);
...
void Ball::update(sf::RenderWindow *window, float dt) {
    ...
    // Moving the position of the ball based on the vector * time in between
    // frames
